...
----
## Data Preparation
The pre-training protein 3D coordinates datasets can be dowloaded from [here](https://alphafold.ebi.ac.uk/download).
...
----
## Models Preparation
You also need to download the following pre-trained models: 
...
In this part, we fine-tune the [pre-trained model](https://drive.google.com/file/d) (i.e., a checkpoint of GLProtein) on various downstream tasks.
...
An example of fine-tuning GLProtein for contact prediction (`script/run_contact.sh`) is as follows:
...
To validate GLProtein on these two tasks, you need to:
- Configure paths in `src/benchmark/PROBE/extract_embeddings.py` to your pre-trained model and PROBE data accordingly.
- Extract embeddings using pre-trained GLProtein by running `src/benchmark/PROBE/extract_embeddings.py`. 
...
To validate GLProtein for PPI prediction:
